...
@pytest.mark.django_db
@pytest.mark.parametrize('blog__published', [True])
def test_full_blog_feed_full_posts(client, site, full_identity, blog, blog_post_factory):
    posts = blog_post_factory.create_batch(3, parent=blog, published=True, full=True, tags=3, categories=3)
    url = blog.url + blog.reverse_subpage('feed')
    response = client.get(url)
